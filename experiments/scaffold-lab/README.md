...
```sh
npm run build:lab
npm --prefix experiments/scaffold-lab run dev
```

## Friction

### npm does not resolve the required workspace protocol
...
### The colour contract has no subtle structural border
...
### The colour contract has no text role for filled actions
...
### Storage uses one mutable process-wide driver
...
### A failed migration does not identify or checkpoint the failed step
